...
template<typename T> 
map<int, T> GenericAPI<T>::resourceMap;
 ...
 */
template<typename T> 
response GenericAPI<T>::readResource(int id) 
{
    try 
    {
...
response GenericAPI<T>::readAllResources() 
{
    ...
    int index = 0;
    for (pair<int, T> keyValuePair : resourceMap)
    {
 ...
 */
template<typename T> 
void GenericAPI<T>::updateResource(request req, response& res, int id) 
{
    try 
    {
        // Get the resource from the resource map.
 ...
 */
template<typename T> 
response GenericAPI<T>::deleteResource(int id) 
{
    try 
    {
        // Get the resource from the resource map.
...
template class GenericAPI<Profiles>;
template class GenericAPI<Destinations>;
template class GenericAPI<Photos>;
template class GenericAPI<Notes>;
template class GenericAPI<Media>;